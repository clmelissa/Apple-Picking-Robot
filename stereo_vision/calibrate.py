...
CHESSBOARD_SIZE = (6,9) #7 rows 10 columns
...
time.sleep(3)
#for i in range(0,len(rightObjectPoints)):

#    if(leftObjectPoints[i] != rightObjectPoints[i]):
...
j = 0
filenames = []
while(j < len(leftFilenames)):
    filenames.append(leftFilenames[j])    
    filenames.append(rightFilenames[j])    
    j = j+1
print(filenames)
time.sleep(3)
...
_, leftCameraMatrix, leftDistortionCoefficients, _, _ = cv2.calibrateCamera( #Changed left and right functions so they used the left and write object points
